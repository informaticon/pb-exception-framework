...
void stack_build_string(callback_state *state, long line_no) {
	wchar_t temp[512];
	...
	if (wcscmp(state->group_name, state->class_name) == 0) {
		if (wcscmp(state->group_name, state->routine_name) == 0) {
			wnsprintf(temp, 512, L"%s: %d", state->routine_name, line_no);
		} else {
			wnsprintf(temp, 512, L"%s.%s: %d", state->group_name, state->routine_name, line_no);
		}
	} else {
		wnsprintf(temp, 512, L"%s.%s.%s: %d", state->group_name, state->class_name, state->routine_name, line_no);
	}
