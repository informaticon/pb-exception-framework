forward
global type exf1_u_exf_application_adapter from nonvisualobject
end type
type st_encrypt_result from structure within exf1_u_exf_application_adapter
end type
end forward
...
global exf1_u_exf_application_adapter exf1_u_exf_application_adapter
...
protected string ps_exception_server_api_key = 'Bearer eDdoc0o4TTgyRlJLNWh3ckpubWhXbw=='
protected string ps_exception_server_url = 'https://metric.informaticon.com/apis/exception/v1/exceptions'
...
or pf_is_inherited_from(apo_object, 'datastore') then
	...
	if apo_object.dynamic getfullstate(lu_blob.ibl_data) < 0 then
		lu_blob.ibl_data = blob('error getullstate() < 0')
	end if
	
	lu_blob.ibo_confidential = pf_is_dataobject_confidential(apo_object)
	
else
	//Typ: sonstiges PowerObject
	if isvalid(apo_object) then
		lu_blob.is_type = classname(apo_object)
	end if
	
	//Daten: String-Repräsentation des Objekts
	try
		lu_blob.ibl_data = blob(apo_object.dynamic of_to_string())
	catch (runtimeerror lre_e)
		lu_blob.ibl_data = blob('No string representation available')
	end try
end if
...
on exf1_u_exf_application_adapter.create
call super::create
TriggerEvent( this, "constructor" )
end on

on exf1_u_exf_application_adapter.destroy
